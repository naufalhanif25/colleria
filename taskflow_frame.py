...
def taskflow_tool(root, frame):
    ...
    task_panel_canvas.grid(row = 0, column = 0, padx = 0, pady = 0, sticky = "nsew")
    ...
    task_panel_canvas.create_window((0, 0), window = task_buttons_frame, anchor = "nw", tags = "task_buttons_frame")

    # Update scroll region and width when the frame size changes 
    def on_configure(event): 
        task_panel_canvas.configure(scrollregion = task_buttons_frame.bbox("all")) 
        task_panel_canvas.itemconfig("task_buttons_frame", width = task_panel_canvas.winfo_width())
        
    # Bind <Configure> event to on_configure function 
    task_panel_canvas.bind("<Configure>", on_configure)
